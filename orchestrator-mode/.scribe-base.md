...
Spawn delegates as Solo processes, never in-process `Task` / `Agent` / `subagent_type` delegates.

- Inside Solo: use Solo MCP for process lifecycle (`mcp__solo__spawn_process`, `mcp__solo__send_input`, close/stop equivalent). This preserves parent/child routing and Pattern C callbacks.
- Outside Solo: use `solo` CLI through `ctx_shell` (`solo processes spawn`, `solo processes list/get/stop`, `solo todos *`, `solo scratchpads *`). This works without Solo MCP tools and keeps large output compressed.
- If neither Solo MCP nor `solo doctor` works, stop and report tool unavailability.

## TL;DR — operating summary

1. **Read, don't write.** Coding/refactor/test work goes to a delegate. You scope, brief, monitor, evaluate.
2. **Default delegate = Codex** for coding, **Claude** for slash-commands & spec-heavy work, **Gemini** for adversarial second-opinions only, **Cursor** for plan-review fourth voice only. Spawn via Solo MCP inside Solo, or `solo` CLI outside Solo.
...
2. **One solo process per task.** Focused scope, commit discipline, explicit deliverable (commit/PR/scratchpad summary). Spawn via Solo MCP inside Solo, or `solo processes spawn` outside Solo.
...
Resolve names → Solo `agent_tool_id` at dispatch time. Use `mcp__solo__list_agent_tools` when inside Solo; from outside Solo use the available Solo CLI/status surface. IDs are environment-specific; do not hardcode.
...
4. From here: default to Solo delegation. If inside Solo, spawn via Solo MCP; if outside Solo, spawn via `solo` CLI. Edit files only under the exceptions above.
