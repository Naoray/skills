---
name: north-star
description: Use to derive, persist, and propagate a "North Star" project compass (mission, non-goals, constraints, principles) for autonomous agents. Trigger via `/north-star`, "set project compass", or during `/orchestrator-mode` boot if missing. Inputs: repo signals (README, manifests, CHANGELOG) + user signoff. Produces `docs/NORTH_STAR.md` and a MemPalace mirror. Do not use for roadmaps, sprint plans, or feature scoping; use planning/strategy skills instead. Escalate on conflicting docs or missing write access. Auto-injected into delegate briefs via orchestrator-mode.
---
