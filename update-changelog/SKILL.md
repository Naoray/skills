...
description: Update CHANGELOG.md with changes since the last release. Analyzes commits, categorizes them using Keep a Changelog format, and adds missing entries to the Unreleased section.
---
...
5. **Compare commits against existing entries**:
   - Read through the `[Unreleased]` section
   - Identify commits that are NOT already captured in the changelog
...
7. **Present the changes** to the user before writing:
   ...
   - Show which category each entry belongs to
...
- Only add categories that have entries (don't add empty `### Fixed` sections)
- Write descriptions that are meaningful to end users
