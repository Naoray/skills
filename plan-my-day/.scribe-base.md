...
- `icalBuddy` — reads all Apple + Google calendars
- `gws calendar` — writes to Google calendars
- `~/context/admin/scripts/sync-reminders.sh` — syncs Apple Reminders Inbox
- Apple Calendar.app skill scripts at `~/.claude/skills/apple-calendar/scripts/`
...
**Filter calendars:** Only show events from relevant calendars. Ignore: Семья (Family), La Ginesta, Feiertage in Deutschland, Kalenderwochen, Bitmate, Deutsche Feiertage, Holidays in Germany, Birthdays, Siri Suggestions.
...
1. **Add time blocks to calendar:**
   - Use Apple Calendar via `~/.claude/skills/apple-calendar/scripts/cal-create.sh` (preferred — ensures native macOS notifications work)
